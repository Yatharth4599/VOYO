...
import { logAgentClick } from "@/lib/Analytics/logAgentClick";
import { getAgentClicks } from '@/lib/Analytics/getAgentClicks';
import { getGlobalClicks } from '@/lib/Analytics/getGlobalClicks';
import SessionTracker from '@/components/Analytics/SessionTracker';
import { getAnonymousUserId } from '@/lib/Analytics/generateAnonId'
import { analytics } from '@/lib/firebase';
import { logAgentView } from '@/lib/Analytics/logAgentView'
import { logSearchQuery } from '@/lib/Analytics/logSearchQuery';
...
export default function Hero() {
  ...
  const [cardGridHeight, setCardGridHeight] = useState(0);
  const [user, setUser] = useState<{name: string, email: string} | null>(null);
  const [agentClickCount, setAgentClickCount] = useState<number | null>(null)
  ...
  const filteredAgents = agents
    ...
    useEffect(() => {
      if (modal.isOpen && modal.agent) {
        getAgentClicks(modal.agent['Agent Name']).then(setAgentClickCount)
      }
    }, [modal])
    ...
    useEffect(() => {
      if (typeof window !== 'undefined') {
        anonIdRef.current = getAnonymousUserId()
      }
    }, [])
    ...
    const logTimeSpent = () => {
      ...
      logEvent('time_on_page', { seconds: duration })
    }
    ...
    return () => {
      ...
      window.removeEventListener('pagehide', logTimeSpent)
    }
  }, [])

    useEffect(() => {
      const timeout = setTimeout(() => {
        if (searchQuery.trim()) logSearchQuery(searchQuery)
      }, 800)
  ...
  return (
    <section className="bg-[#FFFBF3] relative dark:bg-gradient-to-b dark:from-[#120B27] dark:via-orange-950 dark:to-black text-black dark:text-white pt-30 pb-32 overflow-hidden">
      ...
      <SessionTracker userId={user?.email || anonIdRef.current || 'anon'} />
