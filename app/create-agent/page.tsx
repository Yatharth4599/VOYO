...
export default function CreateAgentPage() {
  ...
  const handleCreate = async () => {
    ...
    if (!token) throw new Error('Authentication required');
      ...
      if (form.selectedKnowledgeId) {
        const selectedDoc = knowledgeDocuments.find(doc => doc.id === form.selectedKnowledgeId);
        if (selectedDoc) {
          knowledgeBase.push({
            type: "text",
  ...
  const handleUpdate = async () => {
    ...
    // Step 2: Create the update payload with the correct structure expected by the backend
    // Determine the correct model_id based on language
    const getTTSModel = (language: string) => {
      if (language === 'en') {
        return 'eleven_turbo_v2'; // English requires turbo or flash_v2
      }
      return originalAgentData.conversation_config.tts.model_id; // Keep original for other languages
    };
    ...
    const updatePayload = {
      ...
      conversation_config: {
        ...
        tts: {
          ...
          voice_id: form.voiceId,
          speed: parseFloat(form.speed.toString()),
          model_id: getTTSModel(form.language),
        },
        agent: {
    ...
    // Clear the selected knowledge ID after successful update
    setForm(prev => ({ ...prev, selectedKnowledgeId: '', knowledgeFile: null }));
    
    // Stay on the same page for editing - no redirect
  };
      ...
      {menuOpen && (
        ...
        >
          ...
          <ul className="space-y-4">
            ...
            <li><a href="/phone-numbers" className="block text-lg text-gray-700 hover:text-amber-600 transition-colors">Phone Numbers</a></li>
          </ul>
        </motion.div>
      )}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              ...
              <option value={0.93}>Medium</option>
              <option value={1.20}>Fast</option>
            </select>
          </div>
        </div>
          ...
          <div className="mb-4">
            ...
            {form.selectedKnowledgeId && (
              <p className="text-sm text-amber-600 mt-2">
                ✓ Selected: {knowledgeDocuments.find(doc => doc.id === form.selectedKnowledgeId)?.name}
              </p>
            )}
          </div>
